...
import jp.techacademy.satoko.abiko.apiapp.databinding.RecyclerFavoriteBinding
...
class FavoriteItemViewHolder(private val binding: RecyclerFavoriteBinding) :
    ...
    fun bind(favoriteShop: FavoriteShop, position: Int, adapter: FavoriteAdapter) {
        binding.rootView.apply {
            ...
            // クリック時のイベントリスナーを割り当て
            setOnClickListener {
                adapter.onClickItem?.invoke(favoriteShop.imageUrl,favoriteShop.name,favoriteShop.id,favoriteShop.url,favoriteShop.address)
            }
        }
